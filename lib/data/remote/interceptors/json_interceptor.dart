...
import 'package:challenge/models/index.dart';
...
class JsonInterceptor<ErrorType> implements ApiInterceptor {
  ...
  static dynamic convertToJson<T extends ToJson>(dynamic data) {
    return json.encode(_encode<T>(data));
  }

  static dynamic _serialize<T extends ToJson>(T value) => value.toJson();

  static List<T> _serializeListOf<T extends ToJson>(List value) =>
      List.castFrom(
        value.map((value) => _serialize<T>(value)).toList(),
      );

  static dynamic _encode<T extends ToJson>(entity) {
  ...
  static List<T> _decodeList<T>(List values) =>
  ...
  ApiResponse<ResponseType, InnerType> onResponse<ResponseType, InnerType>(
    ...
    //   pagination = _decode<Pagination>(_getBody(jsn));
    // }
  ...
  ApiResponse<ResponseType, InnerType> onError<ResponseType, InnerType>(
    ...
    try {
      ...
      if ((json as Map<String, dynamic>?)?.getKey('message') is List) {
        var body = _decode<ErrorModel>(json);
        return response.copyWith(error: body);
      }

      // if ((json as Map<String, dynamic>?)?.getKey('message') is String) {
      var body = _decode<ErrorModel>(json);
      return response.copyWith(error: body);
      // }
    } catch (e) {
